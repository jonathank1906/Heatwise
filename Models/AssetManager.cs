...
public class AssetManager
{
    ...
    public void LoadAssetsAndPresetsFromJson()
    {
        try
        {
            ...
            if (root.TryGetProperty("HeatingGrid", out var gridElement))
            {
                GridInfo = JsonSerializer.Deserialize<HeatingGrid>(gridElement.GetRawText());
                Debug.WriteLine($"Loaded heating grid info: {GridInfo?.Name ?? "null"}");
            }
            else
            {
                Debug.WriteLine("No heating grid information found in JSON");
            }
        }
        catch (Exception ex)
        {
